                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-rose-50/10 transition-colors group">
                        <td class="px-8 py-6">
                            ...
                            <div class="text-[9px] font-bold text-gray-400 mt-1 uppercase">{{ \Carbon\Carbon::parse($denda->tanggal_terdeteksi)->format('d M Y') }}</div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="font-bold text-gray-800 text-xs uppercase">{{ $denda->peminjaman->user->name ?? 'N/A' }}</div>
                            <!-- 🔹 Diperbarui: plat_nomor direpresentasikan oleh id mobil -->
                            <div class="text-[10px] mt-0.5 font-mono text-rose-400">{{ $denda->peminjaman->mobil->id ?? '-' }}</div>
                        </td>
                        <td class="px-8 py-6">
                            <div class="flex flex-col gap-1">
                                @if($denda->denda_keterlambatan > 0)
                                    <span class="text-[10px] font-bold text-gray-600 flex justify-between w-32">
                                        <span>Late:</span> <span>Rp {{ number_format($denda->denda_keterlambatan) }}</span>
                                    </span>
                                @endif
                                @if($denda->denda_kerusakan > 0)
                                    <span class="text-[10px] font-bold text-gray-600 flex justify-between w-32">
                                        <span>Damage:</span> <span>Rp {{ number_format($denda->denda_kerusakan) }}</span>
                                    </span>
                                @endif
                            </div>
                        </td>
                        <td class="px-8 py-6 text-center">
                            <div class="font-black text-xs text-rose-600">Rp {{ number_format($denda->total_denda, 0, ',', '.') }}</div>
                        </td>
                        <td class="px-8 py-6 text-center">
                            @php
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex justify-end gap-2">
                                <button wire:click="showDetail({{ $denda->id }})" class="p-2.5 text-rose-600 bg-rose-50 hover:bg-rose-600 hover:text-white rounded-xl transition-all border border-rose-100 shadow-sm" title="Detail">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </button>

                                @if($denda->status == 'belum dibayar')
                                    <!-- 🔹 Diperbarui: update-fine -> update-fines -->
                                    @can('update-fines')
                                    <button wire:click="openPaymentModal({{ $denda->id }})" class="p-2.5 text-emerald-600 bg-emerald-50 hover:bg-emerald-600 hover:text-white rounded-xl transition-all border border-emerald-100 shadow-sm" title="Bayar Denda">
            ...
            <div class="inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl w-full border border-gray-100">
                <div class="px-10 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    <div>
                        ...
                        <h3 class="text-xl font-black text-gray-900 uppercase tracking-tight">Detail Denda #{{ $selectedDenda->id }}</h3>
                ...
                <div class="p-10 space-y-8">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                            ...
                            <p class="font-black text-gray-800 text-sm uppercase">{{ $selectedDenda->peminjaman->user->name }}</p>
                        </div>
                        <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100 text-right">
                            ...
                            <p class="font-black text-gray-800 text-xs uppercase">{{ \Carbon\Carbon::parse($selectedDenda->tanggal_terdeteksi)->format('d M Y') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 border-t border-dashed pt-4">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-black text-gray-800">Rp {{ number_format($selectedDenda->denda_keterlambatan) }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-bold text-gray-500 uppercase">Denda Kerusakan</span>
                            <span class="text-sm font-black text-gray-800">Rp {{ number_format($selectedDenda->denda_kerusakan) }}</span>
                        </div>
                        <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                            <span class="text-sm font-black text-rose-600 uppercase tracking-widest">Total Tagihan</span>
                            <span class="text-2xl font-black text-rose-600">Rp {{ number_format($selectedDenda->total_denda) }}</span>
                        </div>
                    </div>
                ...
                <form wire:submit.prevent="processPayment">
                    ...
                    <div class="p-10 space-y-6">
                        <div class="bg-rose-50 border border-rose-100 p-6 rounded-2xl text-center">
                            ...
                            <!-- 🔹 Diperbarui: $selectedFine -> $selectedDenda -->
                            <p class="text-3xl font-black text-rose-600">Rp {{ number_format($selectedDenda->total_denda) }}</p>
                        </div>
