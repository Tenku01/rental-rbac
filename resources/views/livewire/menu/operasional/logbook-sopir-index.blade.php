<div class="p-8 space-y-10 bg-[#f9fafb] min-h-screen font-inter">
    ...
    <div
        ...
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight leading-none uppercase ">Logbook
                    Aktivitas Sopir</h1>
                <p class="text-sm text-gray-500 font-medium mt-2">Pemantauan riwayat perjalanan dan status operasional
                    driver secara real-time.</p>
            </div>
        </div>

        @can('create-logbook_sopir')
            <button wire:click="create"
                class="bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-700 hover:to-blue-700 text-white font-black py-4 px-10 rounded-2xl shadow-xl shadow-cyan-100 flex items-center transition-all transform hover:scale-105 uppercase tracking-[0.2em] text-[11px] leading-none">
    ...
    {{-- 2. CONTROL BAR --}}
    <div
        class="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex flex-col lg:flex-row items-center justify-between gap-4">
        <div class="relative w-full group flex-1">
            <span
                class="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none text-gray-300 group-focus-within:text-cyan-500 transition-colors">
        ...
        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full text-left">
                <thead class="bg-gray-50/80 border-b border-gray-100">
                    <tr>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">Sopir
                            & Armada</th>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">Waktu
                            Aktivitas</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($logs as $log)
                        <tr class="hover:bg-cyan-50/20 transition-colors group">
                            <td class="px-8 py-6">
                                <div class="font-black text-gray-900 text-sm tracking-tight capitalize">
                                    {{ $log->peminjaman->sopir->nama ?? 'N/A' }}</div>
                                <div
                                    class="text-[10px] font-mono text-cyan-600 font-bold mt-1 uppercase tracking-tighter">
                                    {{ $log->peminjaman->mobil->id ?? 'N/A' }}</div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="text-xs font-black text-gray-700 uppercase">
                                    {{ \Carbon\Carbon::parse($log->tanggal_aktivitas)->format('d M Y') }}</div>
                                <div class="text-[10px] text-gray-400 font-bold mt-1 uppercase">
                                    {{ $log->waktu_log->format('H:i') }} WIB</div>
                            </td>
                            <td class="px-8 py-6">
                                <p class="text-xs text-gray-600 line-clamp-1 max-w-xs font-medium italic">
                                    "{{ $log->deskripsi_aktivitas }}"</p>
                            </td>
                            <td class="px-8 py-6 text-center">
                                ...
                                @endphp
                                <span
                                    class="inline-flex px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border {{ $badge }}">
                            ...
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-3">
                                    <button wire:click="showDetail({{ $log->id }})"
                                        class="p-2.5 text-slate-600 bg-slate-50 hover:bg-slate-600 hover:text-white rounded-xl transition-all border border-slate-100 shadow-sm"
                                        title="Detail">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                    ...
                                    @can('delete-logbook_sopir')
                                        <button
                                            onclick="confirm('Hapus log aktivitas ini secara permanen?') || event.stopImmediatePropagation()"
                    ...
                    @empty
                        <tr>
                            <td colspan="5"
                                class="px-8 py-24 text-center text-gray-400 text-sm font-medium italic uppercase tracking-widest">
                                Belum ada catatan aktivitas sopir.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    class="inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl w-full border border-gray-100">
                    <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}">
                        <div
                            class="px-10 py-6 border-b border-gray-100 bg-white sticky top-0 z-10 flex justify-between items-center">
                            <div>
                                <h3 class="text-2xl font-black text-gray-900 uppercase tracking-tight">
                                    {{ $modalTitle }}</h3>
                                <p
                                    class="text-[11px] text-cyan-600 font-bold uppercase tracking-[0.25em] mt-2 leading-none">
                                    Dokumentasi Operasional Sopir Harian</p>
                            </div>
                        ...
                        <div class="p-10 space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar bg-white">
                            ...
                            @if (!$isEditMode)
                                <div class="group" x-data="{
                                    ...
                                    <label
                                        class="block text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3 ml-1 group-focus-within:text-cyan-600 transition-colors">
                                    ...
                                    @error('peminjaman_id') border-rose-500 bg-rose-50 text-rose-900 focus:ring-4 focus:ring-rose-500/10 @else border border-gray-100 bg-gray-50/50 text-gray-800 focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 @enderror">
                                        ...
                                        <div x-show="open" @click.away="open = false"
                                            class="absolute z-30 mt-2 w-full bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden"
                                            style="display:none;">
                                            <div class="p-3 border-b border-gray-50">
                                                <input x-model="search" type="text"
                                                    class="w-full h-10 rounded-xl border border-gray-100 bg-gray-50 px-4 text-xs font-bold text-gray-700 placeholder-gray-400 focus:border-cyan-500 focus:outline-none transition-all"
                                                    placeholder="Cari Nama Sopir atau Plat...">
                                            </div>
                                            <div class="max-h-52 overflow-y-auto custom-scrollbar">
                                                ...
                                                <div x-show="filtered.length === 0"
                                                    class="p-5 text-center text-[10px] font-bold text-gray-400 uppercase italic">
                                                    Tidak ada sopir yang sedang bertugas.</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div x-show="selected" class="mt-6 animate-fade-in-down" style="display:none;">
                                        <div
                                            class="bg-gradient-to-r from-cyan-50 to-blue-50 px-6 py-5 rounded-2xl shadow-sm border border-cyan-100 flex justify-between items-center">
                                            <div>
                                                <p
                                                    class="text-[9px] text-gray-500 uppercase font-black tracking-widest">
                                                    Mobil Tugas</p>
                                                <p class="text-lg font-black text-cyan-700 mt-1 uppercase"
                                                    x-text="selectedRental ? selectedRental.merek + ' - ' + selectedRental.plat : ''">
                                                </p>
                                            </div>
                                            <div class="text-right">
                                                <p
                                                    ...
                                                    Periode Sewa</p>
                                                <div
                                                    class="flex items-center mt-1 text-xs font-bold text-gray-600 gap-2">
                                    ...
                                    @error('peminjaman_id')
                                        <span
                                            class="text-rose-500 text-[10px] font-black uppercase mt-2 ml-1 flex items-center gap-1 tracking-widest">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="md:col-span-2 group">
                                    ...
                                    <label
                                        class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-[2rem] transition duration-200 cursor-pointer bg-gray-50/30
                                        ...
                                        <div class="space-y-2 text-center">
                                            @if ($foto_bukti)
                                                <img src="{{ $foto_bukti->temporaryUrl() }}"
                                                    class="mx-auto h-32 w-auto object-cover rounded-xl opacity-90 shadow-sm">
                                            @else
                                                <svg class="mx-auto h-12 w-12 transition-colors @error('foto_bukti') text-rose-400 @else text-gray-400 group-hover:text-cyan-500 @enderror"
                                                ...
                                                <div class="flex text-sm text-gray-600 justify-center">
                                                    <div class="relative cursor-pointer rounded-md font-medium">
                                                        <span
                                                            class="px-6 py-2 rounded-xl transition font-black uppercase text-[10px] tracking-widest leading-none
                                                        @error('foto_bukti') bg-rose-100 text-rose-600 hover:bg-rose-200 @else bg-cyan-100 text-cyan-600 hover:bg-cyan-200 @enderror">Pilih
                                                            file Gambar</span>
                                                        <input type="file" wire:model.live="foto_bukti"
                                    ...
                                    <div wire:loading wire:target="foto_bukti"
                                        class="text-[10px] text-cyan-500 font-bold mt-2 ml-1 animate-pulse">Sedang
                                        memproses gambar...</div>
                                    @error('foto_bukti')
                                        <span
                                            class="text-rose-500 text-[10px] font-black uppercase mt-2 ml-1 flex items-center gap-1 tracking-widest">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor"
                        ...
                        <div
                            ...
                            <button type="submit" wire:loading.attr="disabled"
                                ...
                                <span wire:loading.remove
                                    wire:target="store, update">{{ $isEditMode ? 'Simpan Perubahan' : 'Catat Aktivitas' }}</span>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="px-10 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        <div>
                            <h3 class="text-xl font-black text-gray-900 uppercase tracking-tight">Detail Log Aktivitas
                                #{{ $selectedLog->id }}</h3>
                            <p
                                class="text-[10px] text-cyan-600 font-bold uppercase tracking-[0.25em] mt-1 leading-none">
                                Histori Perjalanan Armada</p>
                        </div>
                    ...
                    <div class="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Nama
                                    Sopir</p>
                                <p class="font-black text-gray-800 text-xs uppercase">
                                    {{ $selectedLog->peminjaman->sopir->nama ?? 'N/A' }}</p>
                            </div>
                            <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100 text-right">
                                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Armada
                                </p>
                                <p class="font-black text-gray-800 text-xs uppercase">
                                    {{ $selectedLog->peminjaman->mobil->merek ?? 'N/A' }}</p>
                                <p class="text-[10px] font-mono text-cyan-600 font-bold mt-1 tracking-wider">
                                    [{{ $selectedLog->peminjaman->mobil->id ?? 'N/A' }}]</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-2">
                                <span
                                    class="h-8 w-8 rounded-full flex items-center justify-center text-sm shadow-sm
                                ...
                                <h5 class="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">Status:
                                    {{ str_replace('_', ' ', $selectedLog->status_log) }}</h5>
                            </div>
                            <div
                                class="p-6 bg-cyan-50/50 rounded-[2rem] border border-cyan-100 text-cyan-900 font-bold text-xs leading-relaxed italic shadow-inner">
                        ...
                        @if ($selectedLog->foto_bukti)
                            <div class="space-y-4">
                                <h5
                                    class="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100 pb-2">
                                    Foto Dokumentasi</h5>
                                <img src="{{ asset('storage/' . $selectedLog->foto_bukti) }}"
                                    class="w-full rounded-[2.5rem] shadow-xl border-4 border-white transition-transform hover:scale-[1.02]">
                            </div>
                        @endif
                    </div>
