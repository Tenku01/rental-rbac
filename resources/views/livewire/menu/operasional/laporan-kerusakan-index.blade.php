<div class="p-8 space-y-10 bg-[#f9fafb] min-h-screen font-inter">
    ...
    <div
        ...
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight leading-none uppercase ">Laporan
                    Kerusakan</h1>
                <p class="text-sm text-gray-500 font-medium mt-2">Daftar temuan kerusakan armada dan rincian biaya
                    perbaikan.</p>
            </div>
        </div>

        @can('create-laporan_kerusakan')
                ...
                <thead class="bg-gray-50/80 border-b border-gray-100">
                    <tr>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">
                            Laporan & Unit</th>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">Ref.
                            Pengembalian</th>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">
                            Deskripsi Kerusakan</th>
                        <th
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($reports as $item)
                        <tr class="hover:bg-cyan-50/20 transition-colors group">
                            <td class="px-8 py-6">
                                <div class="font-black text-gray-900 text-sm tracking-tight">{{ $item->kode_laporan }}
                                </div>
                                <div
                            ...
                            <td class="px-8 py-6">
                                <span
                                    class="px-3 py-1.5 bg-gray-100 text-gray-600 text-[10px] font-black rounded-lg border border-gray-200 uppercase tracking-tighter">{{ $item->pengembalian_kode ?? 'N/A' }}</span>
                            </td>
                            <td class="px-8 py-6">
                                <!-- 🔹 Diperbarui: damage_description -> deskripsi_kerusakan -->
                                <p class="text-xs text-gray-600 line-clamp-2 max-w-xs font-medium italic">
                                    "{{ $item->deskripsi_kerusakan }}"</p>
                            </td>
                            <td class="px-8 py-6 text-center">
                                <!-- 🔹 Diperbarui: damage_cost -> biaya_kerusakan -->
                                <div class="text-[13px] font-black text-rose-600">Rp
                                    {{ number_format($item->biaya_kerusakan, 0, ',', '.') }}</div>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-3">
                                    <button wire:click="showDetail('{{ $item->kode_laporan }}')"
                                    ...
                                    @can('update-laporan_kerusakan')
                                        <button wire:click="edit('{{ $item->kode_laporan }}')"
                                    ...
                                    @can('delete-laporan_kerusakan')
                                        <button
                                            onclick="confirm('Hapus laporan kerusakan ini secara permanen?') || event.stopImmediatePropagation()"
                                            wire:click="delete('{{ $item->kode_laporan }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="5"
                                ...
                                Tidak ditemukan laporan kerusakan armada.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                        ...
                        <div
                            ...
                            <div>
                                <h3 class="text-2xl font-black text-gray-900 uppercase tracking-tight">
                                    {{ $isEditMode ? 'Koreksi Kerusakan' : 'Buat Laporan Baru' }}</h3>
                                <p
                                    class="text-[10px] text-cyan-600 font-bold uppercase tracking-[0.25em] mt-1 leading-none">
                                    {{ $isEditMode ? 'ID Laporan: ' . $editingKode : 'Pencatatan Insiden Baru' }}</p>
                            </div>
                        ...
                        <div class="p-10 space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar bg-white">
                            ...
                            @if (!$isEditMode)
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div class="group">
                                        <label
                                            class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-1 group-focus-within:text-cyan-600 transition-colors">
                                    ...
                                    @error('mobil_id') border-rose-500 bg-rose-50 text-rose-900 focus:ring-4 focus:ring-rose-500/10 @else border border-gray-100 bg-gray-50/50 text-gray-800 focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 @enderror">
                                            ...
                                            @foreach ($mobils as $mobil)
                                                <option value="{{ $mobil->id }}">{{ $mobil->merek }}
                                                    ({{ $mobil->id }})</option>
                                            @endforeach
                                        </select>
                                        @error('mobil_id')
                            ...
                            <div class="group">
                                ...
                                <div class="relative">
                                    <div
                                        class="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none text-gray-400 font-black">
                                        Rp</div>
                                    <!-- 🔹 Diperbarui: damage_cost -> biaya_kerusakan -->
                        ...
                        <div
                            ...
                            <button type="submit" wire:loading.attr="disabled"
                                ...
                                <span wire:loading.remove
                                    wire:target="store, update">{{ $isEditMode ? 'Simpan Perubahan' : 'Buat Laporan Baru' }}</span>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="px-10 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                        <div>
                            <h3 class="text-xl font-black text-gray-900 uppercase tracking-tight">Detail Kerusakan:
                                {{ $selectedReport->kode_laporan }}</h3>
                            <p
                                class="text-[10px] text-cyan-600 font-bold uppercase tracking-[0.25em] mt-1 leading-none">
                                Laporan Audit Fisik Kendaraan</p>
                        </div>
                    ...
                    <div class="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
                                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Informasi
                                    Armada</p>
                                <p class="font-black text-gray-800 text-xs uppercase">
                                    {{ $selectedReport->mobil->merek ?? 'Mobil Dihapus' }}</p>
                                <p class="text-[10px] font-mono text-cyan-600 font-bold mt-1 tracking-wider">
                                    [{{ $selectedReport->mobil_id }}]</p>
                            </div>
                            <div class="bg-white p-5 rounded-2xl border border-gray-100 text-right shadow-sm">
                                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Penyewa
                                    Terakhir</p>
                                <p class="font-black text-gray-800 text-xs uppercase">
                                    {{ $selectedReport->pengembalian->peminjaman->user->name ?? 'User Dihapus / Manual' }}
                                </p>
                        ...
                        <div class="space-y-4">
                            <h5
                                ...
                                Deskripsi Kerusakan</h5>
                            <div
                                class="p-6 bg-amber-50/50 rounded-[2rem] border border-amber-100 text-amber-900 font-bold text-xs leading-relaxed italic shadow-inner">
                                <!-- 🔹 Diperbarui: damage_description -> deskripsi_kerusakan -->
                        ...
                        <div
                            class="p-6 bg-rose-50 rounded-[2rem] border-2 border-rose-100 flex justify-between items-center shadow-sm">
                            <div>
                                <p class="text-[10px] font-black text-rose-500 uppercase tracking-[0.2em]">Total Biaya
                                    Ganti Rugi</p>
                            </div>
                            <!-- 🔹 Diperbarui: damage_cost -> biaya_kerusakan -->
                            <p class="text-2xl font-black text-rose-600">Rp
                                {{ number_format($selectedReport->biaya_kerusakan, 0, ',', '.') }}</p>
                        </div>
                    </div>
