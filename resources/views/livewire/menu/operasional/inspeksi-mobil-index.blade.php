<div class="p-8 space-y-10 bg-[#f9fafb] min-h-screen font-inter">
    ...
    <div
        ...
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight leading-none uppercase ">Pengecekan
                    Armada</h1>
                <p class="text-sm text-gray-500 font-medium mt-2">Inspeksi awal sebelum penyerahan dan inspeksi
                    pasca-pengembalian.</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-10">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead class="bg-blue-50/30 border-b border-gray-100">
                            <tr>
                                <th class="px-6 py-4 text-[10px] font-black text-blue-600 uppercase tracking-widest">ID
                                    Pesanan</th>
                                <th class="px-6 py-4 text-[10px] font-black text-blue-600 uppercase tracking-widest">
                                    Detail Pelanggan</th>
                                <th
                                    class="px-6 py-4 text-[10px] font-black text-blue-600 uppercase tracking-widest text-right">
                                    Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                                ...
                                <tr wire:key="pre-{{ $pre->id }}" class="hover:bg-blue-50/10 transition-colors">
                                    <td class="px-6 py-5">
                                        ...
                                        <div class="text-[9px] text-gray-400 font-bold mt-1 uppercase">
                                            {{ \Carbon\Carbon::parse($pre->tanggal_sewa)->format('d M Y') }}</div>
                                    </td>
                                    <td class="px-6 py-5">
                                        <div class="font-bold text-gray-800 text-xs uppercase">
                                            {{ $pre->user->name ?? 'N/A' }}</div>
                                        <div class="font-bold text-cyan-600 text-[10px] mt-0.5">
                                            {{ $pre->mobil->merek ?? 'N/A' }} [{{ $pre->mobil->id ?? 'N/A' }}]</div>
                                    </td>
                                    <td class="px-6 py-5 text-right">
                                        @can('create-inspeksi_mobil')
                                            @if (empty($pre->kondisi_mobil))
                                                <button wire:click="openPreInspection({{ $pre->id }})"
                                                    class="px-5 py-2 bg-blue-500 hover:bg-blue-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-blue-100 active:scale-95 whitespace-nowrap">
                                                    Catat Awal
                                                </button>
                                            @else
                                                <button wire:click="openPreInspection({{ $pre->id }})"
                            ...
                            @empty
                                <tr>
                                    <td colspan="3"
                                        class="px-6 py-12 text-center text-gray-400 text-[10px] font-bold uppercase tracking-widest italic">
                                        Tidak ada armada yang menunggu penyerahan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="space-y-6">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-sm font-black text-gray-800 uppercase tracking-[0.2em]">Menunggu Pengembalian (Akhir)
                </h2>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead class="bg-amber-50/30 border-b border-gray-100">
                            <tr>
                                <th class="px-6 py-4 text-[10px] font-black text-amber-600 uppercase tracking-widest">ID
                                    Kembali</th>
                                <th class="px-6 py-4 text-[10px] font-black text-amber-600 uppercase tracking-widest">
                                    Detail Pelanggan</th>
                                <th
                                    class="px-6 py-4 text-[10px] font-black text-amber-600 uppercase tracking-widest text-right">
                                    Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            @forelse($pendingReturns as $p)
                                <tr wire:key="ret-{{ $p->kode_pengembalian }}"
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="font-bold text-gray-800 text-xs uppercase">
                                            {{ $p->peminjaman->user->name ?? 'N/A' }}</div>
                                        <div class="font-bold text-cyan-600 text-[10px] mt-0.5">
                                            {{ $p->peminjaman->mobil->merek ?? 'N/A' }}
                                            [{{ $p->peminjaman->mobil->id ?? 'N/A' }}]</div>
                                    </td>
                                    <td class="px-6 py-5 text-right">
                                        @can('create-inspeksi_mobil')
                            ...
                            @empty
                                <tr>
                                    <td colspan="3"
                                        class="px-6 py-12 text-center text-gray-400 text-[10px] font-bold uppercase tracking-widest italic">
                                        Antrean pengecekan akhir kosong.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="flex items-center justify-between mb-2">
            ...
            <div class="relative w-80">
                <input wire:model.live.debounce.300ms="search" type="text"
                    class="w-full h-11 pl-11 pr-4 rounded-xl border border-gray-100 bg-white shadow-sm focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 font-bold text-xs transition-all focus:outline-none"
                    placeholder="Cari Plat Nomor Armada...">
                <svg class="absolute left-4 top-3.5 w-4 h-4 text-gray-300" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-50">
                                        ...
                                        <button wire:click="showDetail('{{ $item->kode_pengembalian }}')"
                                            ...
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                ...
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                    stroke-width="2.5"
                        ...
                        @empty
                            <tr>
                                <td colspan="5"
                                    class="px-8 py-24 text-center text-gray-400 text-sm font-medium italic">Belum ada
                                    riwayat inspeksi armada.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div
                    ...
                    <form wire:submit.prevent="storePreInspection">
                        <div
                            ...
                            <div>
                                <h3 class="text-2xl font-black text-gray-900 uppercase tracking-tight">Pengecekan Awal
                                    Kendaraan</h3>
                                <p
                                    class="text-[10px] text-blue-600 font-bold uppercase tracking-[0.25em] mt-1 leading-none">
                                    Inspeksi Sebelum Penyerahan ke Pelanggan</p>
                            </div>
                            <button type="button" wire:click="closePreModal"
                        ...
                        <div class="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
                                        Penyewa</p>
                                    <p class="font-bold text-gray-800 text-sm uppercase">{{ $pre_infoPenyewa ?? '-' }}
                                    </p>
                                </div>
                                <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100">
                                    <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">
                                        Armada</p>
                                    <p class="font-bold text-gray-800 text-sm uppercase">{{ $pre_infoMobil ?? '-' }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="group">
                                <label
                                    class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3 ml-1">
                                ...
                                <p class="text-[10px] text-gray-400 font-bold mt-2 ml-2 italic">*Catatan yang dibuat
                                    oleh pelanggan tidak akan bisa dihapus/diedit di sini.</p>
                                @error('pre_kondisi')
                                    <span
                                        class="text-rose-500 text-[10px] font-black uppercase mt-2 ml-1 flex items-center gap-1 tracking-widest"><svg
                        ...
                        <div
                            ...
                            <button type="submit" wire:loading.attr="disabled"
                                ...
                                <span wire:loading.remove wire:target="storePreInspection">Selesaikan Inspeksi
                                    Awal</span>
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    class="inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl w-full border border-gray-100">
                    <form wire:submit.prevent="store" x-data="{
                        ...
                        <div
                            class="px-10 py-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
                            <div>
                                <h3 class="text-2xl font-black text-gray-900 uppercase tracking-tight">Detail
                                    Pengecekan Armada</h3>
                                <p
                                    class="text-[10px] text-cyan-600 font-bold uppercase tracking-[0.25em] mt-1 leading-none">
                                    Audit Fisik & Kalkulasi Denda Akhir</p>
                            </div>
                        ...
                        <div class="p-10 space-y-8 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div class="bg-indigo-50 p-5 rounded-2xl border border-indigo-100">
                                    <p class="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">
                                        Harga Sewa / Hari</p>
                                    <p class="font-black text-indigo-700 text-sm uppercase">Rp
                                        {{ number_format((float) ($hargaPerHari ?? 0), 0, ',', '.') }}</p>
                                </div>
                            </div>

                            {{-- 2. DENDA KETERLAMBATAN --}}
                            <div
                                class="p-6 rounded-[2rem] border-2 {{ $jamTerlambat > 0 ? 'bg-rose-50 border-rose-100' : 'bg-emerald-50 border-emerald-100' }}">
                                <h4
                                    class="text-[11px] font-black uppercase tracking-widest mb-4 flex items-center gap-2 {{ $jamTerlambat > 0 ? 'text-rose-600' : 'text-emerald-600' }}">
                                ...
                                @if ($jamTerlambat > 0)
                                    <div class="flex justify-between items-center">
                                        <div>
                                            <p class="text-xs font-bold text-rose-700">Terlambat: <span
                                                    class="text-lg font-black">{{ $jamTerlambat }} Jam</span></p>
                                            <p class="text-[9px] text-rose-400 mt-1 uppercase font-black">Rumus: (Harga
                                                Harian x 10%) x Jam Terlambat</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-[10px] font-black text-rose-400 uppercase tracking-widest">
                                                Subtotal Denda Jam</p>
                                            <p class="text-2xl font-black text-rose-600">Rp
                                                {{ number_format((float) ($lateFine ?? 0), 0, ',', '.') }}</p>
                                        </div>
                                    </div>
                                @else
                                    <p class="text-xs font-bold text-emerald-700 uppercase tracking-wide">Pengembalian
                                        Tepat Waktu (Tidak ada denda keterlambatan).</p>
                                @endif
                            </div>
                            ...
                            <div class="p-6 rounded-[2rem] border-2 bg-gray-50 border-gray-100">
                                <div class="flex justify-between items-center mb-6">
                                    ...
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <span
                                            class="text-[10px] font-black text-gray-400 group-hover:text-cyan-600 uppercase tracking-widest">Temukan
                                            Kerusakan?</span>
                                        <div
                                            class="relative inline-block w-12 h-6 transition duration-200 ease-in-out">
                                            <input type="checkbox" wire:model.live="isDamaged"
                                                class="absolute w-full h-full opacity-0 cursor-pointer z-10 peer">
                                            <div
                                                class="w-full h-full bg-gray-200 rounded-full peer-checked:bg-rose-500 transition-colors">
                                            </div>
                                            <div
                                                class="absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-6 shadow-md">
                                            </div>
                                        </div>
                                    </label>
                                </div>
                                ...
                                @if ($isDamaged)
                                    <div
                                        class="space-y-6 animate-fade-in-down border-t border-dashed pt-6 border-gray-200">
                                        <div class="group">
                                            <label
                                                class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3">Deskripsi
                                                Kerusakan <span class="text-rose-500">*</span></label>
                                        ...
                                        <div class="group">
                                            <label
                                                class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3">Estimasi
                                                Biaya Perbaikan (Rp) <span class="text-rose-500">*</span></label>
                                            <div class="relative">
                                                <div
                                                    class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 font-black">
                                                    Rp</div>
                                                <input wire:model.live.debounce.300ms="biaya_kerusakan" type="number"
                                                    min="0" step="1000"
                                ...
                                @else
                                    <p class="text-xs font-bold text-gray-400 italic">Klik toggle di samping jika
                                        ditemukan kerusakan fisik baru.</p>
                                @endif
                            </div>
                            ...
                            <div class="space-y-6 pt-4 border-t border-dashed border-gray-200">
                                ...
                                <div class="space-y-6">
                                    <div class="group">
                                        <label
                                            class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3">Kondisi
                                            Umum Mobil <span class="text-rose-500">*</span></label>
                                        <select wire:model.live="kondisi"
                                            class="w-full h-14 rounded-2xl border border-gray-100 bg-gray-50 px-5 focus:outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 font-black text-xs uppercase tracking-widest transition-all cursor-pointer">
                                        ...
                                        @error('kondisi')
                                            <span
                                                class="text-rose-500 text-[10px] font-black uppercase mt-2 ml-1 flex items-center gap-1 tracking-widest">{{ $message }}</span>
                                        @enderror
                                    </div>

                                    <div class="group">
                                        <label
                                            class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-3">Catatan
                                            Audit Internal <span class="text-rose-500">*</span></label>
                            ...
                            <div
                                class="bg-gray-900 rounded-[2rem] p-8 text-white flex justify-between items-center shadow-2xl relative overflow-hidden mt-6">
                                <div class="absolute -right-10 -top-10 h-32 w-32 bg-cyan-500/10 rounded-full blur-3xl">
                                </div>
                                <div>
                                    <span
                                        class="text-[10px] font-black text-cyan-400 uppercase tracking-[0.4em] block mb-2">TOTAL
                                        AKHIR DENDA</span>
                                    <span class="text-[8px] text-gray-400 uppercase tracking-widest">(Keterlambatan +
                                        Kerusakan)</span>
                                </div>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100 text-center">
                                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Diaudit
                                    Oleh</p>
                                <p class="font-black text-gray-800 text-xs uppercase">
                                    {{ $selectedInspection->pemeriksa->name ?? 'Sistem' }}</p>
                            </div>
                        </div>

                        <div class="space-y-4">
                            <h5 class="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em] border-b pb-2">
                                Catatan Temuan</h5>
                            <div
                    ...
                    <div class="bg-gray-50/80 px-10 py-8 text-right border-t border-gray-100">
                        <button wire:click="closeModal"
                            class="px-10 py-4 bg-gray-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-gray-800 transition-all active:scale-95 shadow-md">Tutup
                            Audit</button>
                    </div>
                </div>
            </div>
