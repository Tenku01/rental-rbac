<div class="p-8 space-y-10 bg-[#f9fafb] min-h-screen font-inter">
    ...
    <div
        ...
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight leading-none uppercase ">Data
                    Resepsionis</h1>
                <p class="text-sm text-gray-500 font-medium mt-2">Kelola akun dan status aktif staf front office.</p>
            </div>
        </div>

        @can('create-resepsionis')
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden mt-6">
        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full text-left">
                <thead class="bg-gray-50/80 border-b border-gray-100">
                    <tr>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">Nama
                            Lengkap</th>
                        <th class="px-8 py-5 text-[11px] font-extrabold text-gray-400 uppercase tracking-[0.15em]">Akun
                            Email</th>
                        <th
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($resepsionis as $item)
                        <tr class="hover:bg-cyan-50/20 transition-colors group">
                            ...
                            <td class="px-8 py-6">
                                <div class="text-xs font-bold text-gray-600">{{ $item->user->email ?? 'No Email' }}
                                </div>
                            </td>
                            ...
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    @can('delete-resepsionis')
                                        <button wire:confirm="Hapus data resepsionis dan akun user ini?"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-8 py-24 text-center text-gray-400 text-sm font-medium italic">
                                Tidak ditemukan data resepsionis.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="px-8 py-8 border-t border-gray-100 bg-gray-50/50">
            {{ $resepsionis->links('components.pagination-info') }}
        </div>
    </div>
                ...
                <div
                    ...
                    <form wire:submit.prevent="{{ $isEditMode ? 'update' : 'store' }}">
                        ...
                        <div class="p-10 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div class="group">
                                ...
                                <input wire:model="nama" type="text"
                                    class="w-full h-14 rounded-2xl border-gray-100 bg-gray-50 px-5 focus:border-cyan-500 font-bold text-gray-800 transition-all placeholder:text-gray-300"
                                    placeholder="Nama Resepsionis">
                                @error('nama')
                                    <span
                                        class="text-rose-500 text-[10px] font-black mt-2 block tracking-widest">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="group">
                                    <label
                                        class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Password
                                        @if ($isEditMode)
                                            (Opsional)
                                        @endif
                                    </label>
                                ...
                                <div class="group">
                                    <label
                                        class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Konfirmasi
                                        Password</label>
                            ...
                            <div class="group">
                                <label
                                    class="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Status
                                    Keaktifan</label>
                                <div class="flex gap-4">
                                    <label class="flex-1 cursor-pointer">
                                        <input type="radio" wire:model="status" value="aktif"
                                            class="peer sr-only">
                                        <div
                                            class="w-full h-12 flex items-center justify-center rounded-2xl border border-gray-200 bg-white text-gray-400 font-bold text-xs uppercase tracking-wider peer-checked:bg-emerald-50 peer-checked:text-emerald-600 peer-checked:border-emerald-200 transition-all shadow-sm">
                                            Aktif
                                        </div>
                                    </label>
                                    <label class="flex-1 cursor-pointer">
                                        <input type="radio" wire:model="status" value="tidak aktif"
                                            class="peer sr-only">
                                        <div
                                            class="w-full h-12 flex items-center justify-center rounded-2xl border border-gray-200 bg-white text-gray-400 font-bold text-xs uppercase tracking-wider peer-checked:bg-rose-50 peer-checked:text-rose-600 peer-checked:border-rose-200 transition-all shadow-sm">
                                            Tidak Aktif
                                        </div>
                                    </label>
                                </div>
                        ...
                        <div
                            ...
                            <button type="submit"
                                class="inline-flex justify-center rounded-2xl px-12 py-4 bg-cyan-600 text-xs font-black text-white hover:bg-cyan-700 shadow-xl shadow-cyan-100 transition-all uppercase tracking-[0.2em] leading-none">
                                Simpan Data
                            </button>
                            <button type="button" wire:click="closeModal"
