    ...
    <nav x-cloak
        ...
        @can('menu_panel_status_sopir')
            @if (isset($sopir) && $sopir)
                <div x-show="sidebarOpen"
                    class="px-4 py-3 border-t border-b border-cyan-700/50 mx-4 mt-4 rounded-lg bg-cyan-700/30 backdrop-blur-sm shadow-inner">
                    <p
                        class="text-[10px] font-black text-cyan-400 uppercase tracking-widest mb-2 flex justify-between items-center">
                        Status Anda
                        <span
                            class="h-2 w-2 rounded-full animate-ping {{ strtolower(trim(str_replace('_', ' ', $sopir->status))) === 'tersedia' ? 'bg-green-400' : (strtolower(trim(str_replace('_', ' ', $sopir->status))) === 'bekerja' ? 'bg-blue-400' : 'bg-red-400') }}"></span>
                    </p>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <label
                            class="relative inline-flex items-center cursor-pointer {{ $isLocked ? 'opacity-50 pointer-events-none' : '' }}">
                            <input type="checkbox" class="sr-only peer" {{ $isChecked ? 'checked' : '' }}
                                {{ $isLocked ? 'disabled' : '' }} wire:click="toggleStatus" wire:loading.attr="disabled">
                            <div
                                class="relative w-9 h-5 bg-gray-600 rounded-full peer peer-checked:bg-green-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-full duration-300 ease-in-out">
                            </div>
                        </label>
                    </div>
                    @if ($isLocked)
        ...
        <div class="py-4 space-y-1">
            ...
            @canany(['menu_dashboard', 'menu_dashboard_sopir', 'menu_dashboard_staff'])
                ...
                @can('menu_dashboard')
                    <a href="{{ route('home') }}" onclick="this.classList.add('opacity-50','pointer-events-none')"
                        class="group flex items-center px-6 py-3 transition-all duration-200
          {{ request()->routeIs('home')
              ? 'bg-cyan-700 border-l-4 border-cyan-400 text-white'
              : 'hover:bg-cyan-700 border-l-4 border-transparent text-gray-200 hover:text-white' }}"
                        title="Dashboard">

                        <!-- ICON -->
                        <svg class="h-6 w-6 flex-shrink-0 transition-transform duration-200 group-hover:scale-110"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l-2-2m-2 2h-4m-7 20h14a1 1 0 001-1V12a1 1 0 00-1-1H5a1 1 0 00-1 1v7a1 1 0 001 1z">
                            </path>
                        </svg>
        ...
        @canany(['menu_inspeksi', 'menu_laporan_kerusakan', 'menu_logbook_sopir', 'menu_logbook_admin',
            'menu_sanksi_denda'])
            <div class="px-6 mt-6 mb-1 h-4">
                <p x-show="sidebarOpen" x-transition:enter="transition opacity ease-out duration-300"
                    class="text-[10px] font-black text-cyan-400 uppercase tracking-widest whitespace-nowrap">
...
x-transition:leave-end="opacity-0" class="fixed inset-0 z-20 bg-cyan-900/50 lg:hidden"
